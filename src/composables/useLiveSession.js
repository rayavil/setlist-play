...
export function useLiveSession() {
    ...
    function joinSession(selectedRole, roomId) {
        ...
        status.value = selectedRole === 'leader' ? 'Transmitiendo (Líder)' : 'Esperando comandos...';
                ...
                .on('broadcast', { event: 'sync' }, ({ payload }) => {
                    ...
                    if (role.value === 'leader' && payload.type === 'ping') {
                         channel.send({
                            type: 'broadcast',
                            event: 'sync',
                            payload: { type: 'pong', leaderTime: Date.now(), targetId: payload.from }
                        });
                    }

                    // 2. Follower Logic: Process Pong
                    if (role.value === 'follower' && payload.type === 'pong' && payload.targetId === myId) {
                        const now = Date.now();
                        ...
                        if (samples.length < SAMPLE_COUNT) {
                            ...
                            console.log("SYNC COMPLETE. Samples:", samples.map(s => s.rtt), "Final Offset:", offset.value);
                            samples = []; // Reset
                        }
                    }
                    
                    // Pass Commands
                    if (role.value === 'follower' && onSyncCommand.value && payload.type !== 'ping' && payload.type !== 'pong') {
